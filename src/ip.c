...
void traiter_ipv4(const u_char *paquet, int verbosite) {
    ...
    if (verbosite == 2) {
        printf("Adresse IP source : %s - ", inet_ntoa(ip_header->ip_src));
    ...
    // On traiter les protocoles des couches supérieures
...
void traiter_ipv6(const u_char *paquet, int verbosite) {
    ...
    // Verbosité de niveau 2
    if (verbosite == 2) {
        printf("Adresse IP source : %s - ", adresse_source);
        printf("Adresse IP destination : %s\n", adresse_destination);
    }
    ...
    // On traiter les protocoles des couches supérieures
    traiter_protocoles(paquet + sizeof(struct ip6_hdr),
                       ntohs(ip6_header->ip6_plen), verbosite,
                       ip6_header->ip6_nxt, VERSION_IPV6);
}

void traiter_protocoles(const u_char *paquet, int taille, int verbosite,
                        int protocole, int version) {
    switch (protocole) {
    case IPPROTO_UDP:
